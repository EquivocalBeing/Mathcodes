...
INTENSITY_THRESHOLD = 10            ## raise to suppress more noise (post-filter)
FILTER_TYPE         = 'bandpass'    ## None | 'lowpass' | 'highpass' | 'bandpass'
FILTER_SIZE         = 5             ## kernel size — larger = more aggressive
SCALE = 'z'                      ## z == colorbar based on height | intensity == colorbar bar based on signel strength

# Spatial crop in metres — set any to None to disable.
# Run with all None first, check diagnostics, then tighten to suit.
XLIM = (-6,0.5)
YLIM = (-1.5,3)
ZLIM = (-1.2,0)
# ──────────────────────────────────────────────────────────
...
def plot_point_cloud_interactive(points, title="360° Sonar Point Cloud"):
    ...
    if SCALE == 'z' or 'Z':
        color_values = z
        color_label  = 'Z Depth [m]'
    else:
        color_values = intensity
        color_label  = 'Intensity [NA]'
